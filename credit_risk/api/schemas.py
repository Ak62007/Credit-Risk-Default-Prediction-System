...
class RequestModel(BaseModel):
    ...
    acc_now_delinq: float = Field(
        ..., description="Number of accounts currently delinquent.", examples=[0.0, 1.0]
    )
    ...
    avg_cur_bal: float = Field(
        ..., description="Average current balance across all accounts.", examples=[4809.0, 12602.0]
    )
    ...
    chargeoff_within_12_mths: float = Field(
        ..., description="Number of charge-offs in the last 12 months.", examples=[0.0, 1.0]
    )
    ...
    mort_acc: float = Field(..., description="Number of mortgage accounts.", examples=[0.0, 2.0])
    ...
    num_actv_bc_tl: float = Field(
        ..., description="Number of active bankcard (credit card) accounts.", examples=[2.0, 5.0]
    )
    ...
    num_bc_sats: float = Field(
        ..., description="Number of satisfactory bankcard accounts.", examples=[2.0, 6.0]
    )
    ...
    num_op_rev_tl: float = Field(
        ..., description="Number of open revolving accounts.", examples=[3.0, 8.0]
    )

    num_rev_accts: float = Field(
        ..., description="Total revolving accounts.", examples=[8.0, 20.0]
    )
    ...
    num_sats: float = Field(
        ..., description="Number of satisfactory (paid as agreed) accounts.", examples=[8.0, 15.0]
    )
    ...
    num_tl_30dpd: float = Field(
        ..., description="Number of accounts currently 30+ days past due.", examples=[0.0, 1.0]
    )
    ...
    num_tl_op_past_12m: float = Field(
        ..., description="Number of accounts opened in the last 12 months.", examples=[1.0, 4.0]
    )
    ...
    percent_bc_gt_75: float = Field(
        ..., description="% of bankcards with utilization above 75%.", examples=[0.0, 33.3]
    )
    ...
    tax_liens: float = Field(
        ..., description="Number of tax liens against the borrower.", examples=[0.0, 1.0]
    )
    ...
    total_bc_limit: float = Field(
        ..., description="Total credit limit across bankcards.", examples=[10000.0, 28000.0]
    )
    ...
    term: Literal[" 36 months", " 60 months"] = Field(
        ..., description="Loan duration.", examples=[" 36 months"]
    )
    ...
    emp_length: Literal[
        ...
        "9 years",
        "10+ years",
    ] = Field(..., description="How long the borrower has been employed.", examples=["5 years"])

    home_ownership: Literal["MORTGAGE", "RENT", "OWN", "OTHER", "ANY", "NONE"] = Field(
        ..., description="Living situation.", examples=["MORTGAGE"]
    )

    verification_status: Literal["Verified", "Source Verified", "Not Verified"] = Field(
        ..., description="Whether LC verified the income claim.", examples=["Source Verified"]
    )
    ...
    purpose: Literal[
        ...
        ..., description="Borrower's stated reason for the loan.", examples=["debt_consolidation"]
    )
    ...
    addr_state: Literal[
        ...
        "WY",
        "DC",
    ] = Field(..., description="Borrower's US state.", examples=["CA"])

    initial_list_status: Literal["w", "f"] = Field(
        ..., description="The loan's initial listing status.", examples=["w"]
    )
...
class ResponseModel(BaseModel):
    ...
    prob: float = Field(..., description="Predicted probability of default", ge=0.0, le=1.0)
