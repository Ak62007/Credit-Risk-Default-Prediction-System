...
def drop_columns(df: pd.DataFrame, drop_cols: list[str] = DROP_COLS) -> pd.DataFrame:
    ...
    n_df = df.drop(columns=drop_cols, errors="ignore")
...
def prep_one_split(
    df: pd.DataFrame, drop_cols: list[str] = DROP_COLS
...
def building_features(
    ...
    pd.DataFrame(t_train_feat, columns=feature_names).to_parquet(
        output_path / "train_features.parquet"
    )
    pd.DataFrame(t_val_feat, columns=feature_names).to_parquet(
        output_path / "val_features.parquet"
    )
    pd.DataFrame(t_test_feat, columns=feature_names).to_parquet(
...
def load_features(path: Path = FEATURES_DIR) -> dict[str, tuple[pd.DataFrame, pd.DataFrame]]:
    ...
    return {"train": (train_feat, y_train), "val": (val_feat, y_val), "test": (test_feat, y_test)}
