...
def load_data(path: Path) -> pd.DataFrame:
    ...
    raw_data[parse_dates] = raw_data[parse_dates].apply(
...
def compute_snapshot_date(data: pd.DataFrame) -> pd.Timestamp:
    ...
    assert pd.notna(ss_date), "snapshot date is NaT — check issue_d column!"
    return ss_date


def apply_observation_window(data: pd.DataFrame, W: int, ss_date: pd.Timestamp) -> pd.DataFrame:
    ...
    loan_age_months = ss_date.to_period("M").ordinal - data["issue_d"].dt.to_period("M").astype(
        int
    )
    filtered_idx = loan_age_months[loan_age_months >= W].index

    logger.info(
        f"Output: After filtering {len(data.loc[filtered_idx])} rows and {data.shape[1]} columns survived!"
    )
...
def build_target(data: pd.DataFrame) -> pd.DataFrame:
    ...
    assert set(data["target"].unique()) == {0, 1}, (
        "target is not set properly, has more values apart from {0,1}"
    )
...
def make_splits(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    training_mask = (data["issue_d"] >= datetime(year=SPLIT_YEARS["train"][0], month=1, day=1)) & (
        data["issue_d"] <= datetime(year=SPLIT_YEARS["train"][1], month=12, day=31)
    )
    val_mask = (data["issue_d"] >= datetime(year=SPLIT_YEARS["val"][0], month=1, day=1)) & (
        data["issue_d"] <= datetime(year=SPLIT_YEARS["val"][1], month=12, day=31)
    )
    test_mask = (data["issue_d"] >= datetime(year=SPLIT_YEARS["test"][0], month=1, day=1)) & (
        data["issue_d"] <= datetime(year=SPLIT_YEARS["test"][1], month=12, day=31)
    )
    ...
    logger.info(
        f"Splited the data: Training data shape: {training_set.shape} with default rate: {(training_set['target'].sum() / len(training_set)) * 100:.2f}%, Val data shape: {val_set.shape} with default rate: {(val_set['target'].sum() / len(val_set)) * 100:.2f}%, Testing data shape: {test_set.shape} with default rate: {(test_set['target'].sum() / len(test_set)) * 100:.2f}%"
    )
...
def build_dataset(
    ...
    expected_files = [
        processed_dir / name
        for name in [TRAIN_FILENAME, VAL_FILENAME, TEST_FILENAME, METADATA_FILENAME]
    ]
    ...
    if cache_complete and not force_rebuild:
        ...
        assert (processed_dir / TRAIN_FILENAME).exists(), (
            "training_set not created successfully!, File not present"
        )
        assert (processed_dir / VAL_FILENAME).exists(), (
            "val_set not created successfully!, File not present"
        )
        assert (processed_dir / TEST_FILENAME).exists(), (
            "test_set not created successfully!, File not present"
        )
        assert (processed_dir / METADATA_FILENAME).exists(), (
            "metadata not created sucessfully!, File not present"
        )
...
def load_splits(path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    files = [
        path / name for name in [TRAIN_FILENAME, VAL_FILENAME, TEST_FILENAME, METADATA_FILENAME]
    ]
    ...
    if files_exits:
        ...
        logger.info(
            f"Loaded sucessfully all the splits and the metadata, Train_df shape: {train_df.shape}, val_df shape: {val_df.shape}, test_df shape: {test_df.shape}"
        )
        return (train_df, val_df, test_df, metadata)
    else:
        raise FileNotFoundError(
            f"One of the splits or the metadata file not found, Please check {path}"
        )
