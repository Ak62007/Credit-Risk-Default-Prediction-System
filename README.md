...
The full system is deployed on AWS at **https://credit-risk.adii.codes/** — the `/predict` API, the ops dashboard, and a separate public inference demo all run together on a single EC2 instance behind nginx, which terminates TLS (Let's Encrypt via Certbot) and handles path-based routing (`/` to the dashboard, `/api/` to the inference service, `/dashboard-api/` to the dashboard's backend, `/try-the-model/` to the public demo). Every service is bound to `127.0.0.1` internally — nginx is the only thing actually reachable from the public internet.

Try it directly, no login required:
- **[credit-risk.adii.codes/try-the-model](https://credit-risk.adii.codes/try-the-model/)** — submit a simplified loan application and get a live prediction + reason codes from the real deployed model.
- **[credit-risk.adii.codes](https://credit-risk.adii.codes/)** — the ops dashboard (traffic, drift monitoring, score distribution, prediction logs).

It runs **on-demand rather than always-on**: spun up before a demo/interview and stopped afterward, rather than left running continuously — a deliberate cost trade-off (an always-on hobby deployment accrues real AWS charges for no benefit when nobody's looking at it; this project tracks that kind of reasoning explicitly elsewhere, see the accepted-risk write-ups in `references/data_card.md`). **If either link above isn't responding, the instance is simply stopped — reach out and I'm happy to spin it up.** All services auto-start via systemd the moment the instance boots, so starting it is a single step, not a manual multi-service startup.

Getting everything running on a memory-constrained `t3.small` instance (2GB RAM) surfaced a few genuine engineering problems along the way — among them, the drift-monitoring feature initially OOM-killed the dashboard backend by loading the full 466k-row training set into memory just to compute PSI comparisons; the fix was switching to a representative 40k-row sample, which is closer to how real production drift monitors actually work (they rarely diff against a full historical dataset live) than a workaround. Adding the custom domain and TLS surfaced two more: a mixed-content bug from a build-time-baked API URL still pointing at plain HTTP, and an nginx/Next.js redirect loop caused by the two layers disagreeing about a URL's canonical trailing-slash form. The full list of what broke and how it was diagnosed and fixed lives in [`references/deployment_runbook.md`](references/deployment_runbook.md), along with the exact start/stop sequence.
